...
// Pages site (https://savagemanage.github.io/open-games/whiteout/), so a
// production build must resolve assets under '/open-games/whiteout/'.
// During local dev we use '/' so the Vite dev server serves from the root.
export default defineConfig(({ command }) => ({
  base: command === 'build' ? '/open-games/whiteout/' : '/',
  build: {
    outDir: 'dist',
    assetsDir: 'assets',
    sourcemap: false,
